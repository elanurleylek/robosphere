...
// --- React ve Hook Importları ---
import React, { useState, useEffect, useMemo, useRef, useCallback } from 'react';

// --- Router Importları ---
import { Link } from 'react-router-dom';

// --- Shadcn UI ve Diğer Component Importları ---
...
// --- Backend Veri Tipi ve API Importu ---
...
const Courses: React.FC = () => {
  ...
  const fetchCourses = useCallback(async () => {
    // console.log("Fetching courses..."); // Geliştirme için
    setLoading(true);
    setError(null);
    try {
      const data = await courseApi.getAll();
      // console.log("Fetched data from API:", data); // Geliştirme için
      setCourses(data || []);
    } catch (err) {
      const apiErr = err as ApiError;
      console.error("Error fetching courses:", apiErr.details || apiErr.message || err);
      setError(apiErr.message || 'Kurslar yüklenemedi.');
    } finally {
      setLoading(false);
    }
  }, []); 
  ...
  // useEffect(() => {
  //   if(!loading && courses) { 
  //       console.log("Courses state updated, length:", courses.length); // Geliştirme için
  //   }
  // }, [courses, loading]);
  ...
  const handleLevelChange = (value: string) => {
    setCourseFormData(prev => ({...prev, level: value as CourseCreatePayload['level'] }));
  };

  const handleImageFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files && e.target.files[0]) {
      setCourseImageFile(e.target.files[0]);
      setCourseFormData(prev => ({ ...prev, imageUrl: '' })); // URL'yi temizle ki dosya öncelikli olsun
      setRemoveCurrentImage(false);
    } else {
      setCourseImageFile(null);
      // Eğer dosya seçimi iptal edilirse ve düzenleme modundaysa, eski imageUrl'yi geri yükle (remove işaretli değilse)
      if (editingCourse && !removeCurrentImage && editingCourse.imageUrl) {
        setCourseFormData(prev => ({ ...prev, imageUrl: editingCourse.imageUrl }));
      }
    }
  };
  ...
  const resetForm = () => {
    ...
    setCourseFormData({
      ...
      duration: undefined, category: '', imageUrl: null,
      price: null, level: "Belirtilmemiş",
    });
    setCourseImageFile(null);
    setRemoveCurrentImage(false);
  ...
  const handleEditClick = (course: Course) => {
    ...
    setCourseFormData({
      ...
      level: course.level ?? "Belirtilmemiş",
    });
    setIsModalOpen(true);
  };
  ...
  const handleAddOrUpdateCourse = async (e: React.FormEvent) => {
    ...
    if (!courseFormData.title?.trim() || !courseFormData.description?.trim() || !courseFormData.instructor?.trim() || !courseFormData.category?.trim()) {
      setFormError('Lütfen tüm zorunlu alanları (* ile işaretli) doldurun: Başlık, Açıklama, Eğitmen, Kategori.');
      toast({ title: "Eksik Bilgi", description: 'Başlık, Açıklama, Eğitmen ve Kategori zorunludur.', variant: 'destructive' });
      return;
    }
    if (courseFormData.duration !== undefined && (typeof courseFormData.duration !== 'number' || courseFormData.duration <= 0)) {
        setFormError('Süre geçerli bir sayı olmalı ve 0\'dan büyük olmalıdır.');
        toast({ title: "Geçersiz Süre", description: 'Süre pozitif bir sayı olmalıdır.', variant: 'destructive' });
        return;
    }
    if (courseFormData.price !== null && courseFormData.price !== undefined && (typeof courseFormData.price !== 'number' || courseFormData.price < 0)) {
        setFormError('Fiyat geçerli bir sayı olmalı ve 0 veya daha büyük olmalıdır.');
        toast({ title: "Geçersiz Fiyat", description: 'Fiyat pozitif bir sayı veya 0 olmalıdır.', variant: 'destructive' });
        return;
    }
    ...
    if (courseImageFile) {
      ...
      uploadFormData.append('image', courseImageFile);
      const headers: HeadersInit = {};
      if (userInfo.token) { headers['Authorization'] = `Bearer ${userInfo.token}`; }
      try {
        const uploadResponse = await fetch(`${STATIC_FILES_DOMAIN}/api/upload`, {
          method: 'POST', body: uploadFormData, headers: headers,
        });
        if (!uploadResponse.ok) {
          const errorData = await uploadResponse.json().catch(() => ({ message: `HTTP ${uploadResponse.status}` }));
          throw new Error(errorData.message || `Resim yüklenemedi (Durum: ${uploadResponse.status})`);
        }
        const uploadData = await uploadResponse.json();
        if (!uploadData.imageUrl) {
          throw new Error('Resim yüklendi ancak URL alınamadı. Backend yanıtını kontrol edin (imageUrl alanı eksik).');
        }
        resolvedImageUrl = uploadData.imageUrl;
      } catch (uploadError) {
        const errMsg = uploadError instanceof Error ? uploadError.message : 'Bilinmeyen bir resim yükleme hatası.';
        setFormError(`Resim yükleme hatası: ${errMsg}`);
        toast({ title: "Resim Yükleme Başarısız", description: errMsg, variant: 'destructive' });
        setSubmitting(false); return;
      }
    } else {
      if (editingCourse) {
        resolvedImageUrl = removeCurrentImage ? null : (courseFormData.imageUrl === editingCourse.imageUrl ? editingCourse.imageUrl : courseFormData.imageUrl);
      }
    }
    ...
    const payloadBase = {
      ...
      ...(courseFormData.price !== undefined && { price: courseFormData.price }), 
      level: courseFormData.level || "Belirtilmemiş",
    };
    
    try {
      let resultCourse: Course;
      if (editingCourse && editingCourse._id) {
        const updatePayload: CourseUpdatePayload = payloadBase;
        resultCourse = await courseApi.update(editingCourse._id, updatePayload);
        setCourses(prev => prev.map(c => c._id === resultCourse._id ? resultCourse : c));
        toast({ title: 'Başarıyla Güncellendi!', description: `"${resultCourse.title}" güncellendi.` });
      } else {
        const createPayload: CourseCreatePayload = payloadBase as CourseCreatePayload;
        resultCourse = await courseApi.create(createPayload);
        setCourses(prev => [resultCourse, ...prev]);
        toast({ title: 'Başarıyla Eklendi!', description: `"${resultCourse.title}" eklendi.` });
      }
      setIsModalOpen(false); // Formu kapat
    } catch (err) {
        const apiErr = err as ApiError;
        setFormError(`API Hatası: ${apiErr.message || 'Bilinmeyen bir hata'}`);
        toast({ title: "İşlem Başarısız", description: apiErr.message || 'Bilinmeyen bir hata', variant: "destructive" });
    } finally {
      setSubmitting(false);
    }
  };
  ...
  const imagePreviewUrl = useMemo(() => {
    ...
    if (courseFormData.imageUrl && !removeCurrentImage) { // removeCurrentImage kontrolü eklendi
  ...
  // console.log("Render check - courses array length:", courses ? courses.length : 'undefined');
  // console.log("Render check - displayedCourses array length:", displayedCourses ? displayedCourses.length : 'undefined');
  ...
  return (
    <AlertDialog open={isDeleteDialogOpen} onOpenChange={setIsDeleteDialogOpen}>
      <div className="min-h-screen flex flex-col bg-background">
        ...
        <main className="flex-1">
          <section className="bg-gradient-to-b from-primary/10 to-background py-16 md:py-24">
            <div className="container px-6 mx-auto text-center">
              <h1 className="text-4xl md:text-5xl font-bold mb-6 text-foreground">Robotik Eğitim Kursları</h1>
          ...
          <section className="py-12">
            <div className="container px-6 mx-auto">
              <h2 className="text-3xl font-semibold mb-8 text-center text-foreground">Kurs Kategorileri</h2>
          ...
          <section className="py-12 bg-muted/20 dark:bg-muted/5">
            <div className="container mx-auto px-6">
              <div className="flex flex-col sm:flex-row justify-between items-center mb-8 gap-4">
                <h2 className="text-3xl font-semibold text-foreground text-center sm:text-left">Tüm Kurslar</h2>
                {userInfo?.token && (
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                  {displayedCourses.map(course => (
                    <div key={course._id} className="bg-card rounded-xl overflow-hidden shadow-md border border-border h-full flex flex-col hover:shadow-xl hover:border-primary/30 transition-all duration-300 group relative">
                      <Link to={`/courses/${course._id}`} className="block aspect-[16/10] overflow-hidden"> <img src={course.imageUrl ? (course.imageUrl.startsWith('http') ? course.imageUrl : `${STATIC_FILES_DOMAIN}${course.imageUrl}`) : `https://via.placeholder.com/400x250/E0E0E0/B0B0B0?text=${encodeURIComponent(course.title)}`} alt={course.title} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-300" onError={(e) => { (e.target as HTMLImageElement).src = `https://via.placeholder.com/400x250?text=Görsel+Hatalı`; }} /> </Link>
                      <div className="p-4 flex flex-col flex-grow">
                        <Link to={`/courses/${course._id}`} className="block"> <h3 className="text-md font-semibold mb-2 text-foreground hover:text-primary transition-colors line-clamp-2 leading-tight h-[2.75em]">{course.title}</h3> </Link>
                      ...
                      {userInfo?.token && ( <div className="absolute top-2 right-2 flex gap-1.5 opacity-0 group-hover:opacity-100 transition-opacity duration-200 bg-card/80 backdrop-blur-sm p-1 rounded-md z-10"> <Button variant="outline" size="icon" onClick={(e) => { e.stopPropagation(); handleEditClick(course); }} className="h-7 w-7"> <Edit className="h-3.5 w-3.5" /> </Button> 
                        {/* DÜZELTME BURADA: AlertDialogTrigger'a asChild eklendi */}
          ...
          <section className="py-16 bg-gradient-to-t from-primary/10 to-background">
            <div className="container px-6 mx-auto text-center">
              <h2 className="text-3xl font-bold mb-4 text-foreground">Hayalinizdeki Robotiğe Başlayın</h2>
        ...
        <Footer />
        <AlertDialogContent> <AlertDialogHeader> <AlertDialogTitle>Kursu Silmek İstediğinize Emin Misiniz?</AlertDialogTitle> <AlertDialogDescription> Bu işlem geri alınamaz. "{courses.find(c=>c._id === deletingCourseId)?.title || 'Seçili kurs'}" adlı kurs kalıcı olarak silinecektir. </AlertDialogDescription> </AlertDialogHeader> <AlertDialogFooter> <AlertDialogCancel disabled={isProcessingDelete}>İptal</AlertDialogCancel> <AlertDialogAction onClick={confirmDeleteCourse} disabled={isProcessingDelete} className="bg-destructive hover:bg-destructive/90 text-destructive-foreground"> {isProcessingDelete ? <><Loader2 className="mr-2 h-4 w-4 animate-spin" />Siliniyor...</> : "Evet, Sil"} </AlertDialogAction> </AlertDialogFooter> </AlertDialogContent>
      </div>
    </AlertDialog>
  );
};
