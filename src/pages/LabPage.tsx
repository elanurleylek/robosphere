...
const LabPage: React.FC = () => {
  ...
  const handlePlayAnimation = (animationName: string) => {
    setAnimationCommand(animationName);
    setTimeout(() => setAnimationCommand(null), 100);
  };

  const handleStopAnimations = () => {
    setAnimationCommand("STOP_ALL");
    setTimeout(() => setAnimationCommand(null), 100);
  };
  ...
  const toggleFullScreen = () => {
    const elem = simulationContainerRef.current;
    if (!elem) return;
  ...
  return (
    <div className="min-h-screen flex flex-col">
      <Header />
      <main className="flex-1 flex flex-col items-center py-8 px-4 md:px-6 bg-gray-50 dark:bg-gray-900">
        <h1 className="text-3xl md:text-4xl font-bold text-gray-800 dark:text-gray-100 text-center mb-6">Robotik Lab Ortamı</h1>
        ...
        <div className="w-full max-w-6xl flex flex-col lg:flex-row gap-8">
            ...
            >
                {/* <<<--- DÜZELTME: PROPLAR BURAYA EKLENDİ --->>> */}
                <RobotSimulation
            ...
            <div
              className={`lg:w-1/3 lg:max-w-xs w-full p-6 bg-white dark:bg-gray-800 rounded-lg shadow-lg
                          ${isFullScreen ? 'hidden' : ''}`}
            >
                <h2 className="text-xl font-semibold mb-6 text-center text-gray-800 dark:text-gray-100">Kontroller</h2>
                <div className="space-y-6">
                    ...
                    {animationNames.length > 0 && (
                        <div className="pt-4 border-t border-gray-200 dark:border-gray-700">
                            <h3 className="text-md font-semibold mb-3 text-gray-700 dark:text-gray-300">Animasyonlar</h3>
                            <div className="space-y-2">
