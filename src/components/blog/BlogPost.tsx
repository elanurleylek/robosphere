...
// AlertDialog importları burada kalmalı
import {
...
import { Avatar, AvatarFallback, AvatarImage } from "@/components/ui/avatar"; // Avatar component import edildi

import {
  BlogPost as BlogPostType,
  BlogPostAuthor, // BlogPostAuthor tipi types.ts'ten gelmeli
  ApiError,
  BlogCreatePayload, BlogUpdatePayload // Payload tipleri
} from '@/lib/types'; // <-- types dosyasından import edildi


// types.ts dosyanızdaki BlogPostAuthor tanımına (username, avatar?) uygun olacak şekilde güncellendi
export interface BlogPostCardData {
  _id: string; title: string; excerpt: string; author: string; authorId?: string; authorAvatar?: string | null; // <-- authorAvatar tipine null eklendi
  date: string; category: string; image: string; link: string;
}

export interface SidebarPostData {
  _id: string; title: string; date: string; image: string; link: string;
}
...
const BlogSection: React.FC = () => {
  ...
  const fetchBlogPosts = async () => {
   ...
   const resetForm = useCallback(() => {
    ...
    if (imageInputRef.current) imageInputRef.current.value = ""; setFormError(null); setEditingPost(null);
  }, []);
  ...
  const handleEditClick = (postToEdit: BlogPostType) => {
    resetForm(); setEditingPost(postToEdit); setFormTitle(postToEdit.title); setFormContent(postToEdit.content); setFormCategory(postToEdit.category || '');
    setFormTags(postToEdit.tags ? postToEdit.tags.join(', ') : ''); setFormImageFile(null); setRemoveCurrentImage(false); if (imageInputRef.current) imageInputRef.current.value = "";
    const formCard = document.getElementById("blog-form-card"); if (formCard) formCard.scrollIntoView({ behavior: "smooth", block: "start" });
  };
  ...
  const handleAddOrUpdateBlogPost = async (e: FormEvent) => {
    ...
    // Tags'i backend'e dizi olarak gönderirken her tag'i ayrı append et
    ...
    formData.append('excerpt', excerptValue); // Excerpt backend'e gönderilir

    // Resim yükleme veya kaldırma mantığı
    if (formImageFile) {
       formData.append('image', formImageFile, formImageFile.name); // Yeni dosya yükle (backend multer ile 'image' alanını yakalar)
    } else if (editingPost) {
       if (removeCurrentImage) {
           formData.append('removeImage', 'true'); // Mevcut resmi kaldır bayrağını gönder
       }
    }
    ...
    try {
      ...
      resetForm(); // Modalı kapatmıyor, sadece formu resetliyor

      // fetchBlogPosts(); // Yazılar güncellendikten/eklendikten sonra listeyi yeniden çek
  ...
  // getAuthorDetails fonksiyonu, backend populate ettiğinde BlogPostAuthor objesi alabilir
  // types.ts'teki BlogPostAuthor tanımına (username, avatar?) uygun okuyacak şekilde güncellendi
  const getAuthorDetails = useCallback((authorData: BlogPostType['author']): { id?: string; name: string; avatar?: string | null } => {
    ...
    if (typeof authorData === 'string') { // Eğer populate edilmemiş sadece ID gelirse
       return { id: authorData, name: `Yazar ID: ${authorData.substring(0,6)}...`, avatar: null }; // Avatar yok
    }
    // Eğer populate edilmiş obje gelirse (BlogPostAuthor tipinde)
    // types.ts'te BlogPostAuthor'da username ve avatar (ve opsiyonel name) alanı olduğunu varsayıyoruz
    const populatedAuthor = authorData as BlogPostAuthor; // Type assertion burada uygun olabilir, tip types.ts'ten gelmeli
    return {
      id: populatedAuthor._id,
      name: populatedAuthor.username || populatedAuthor.name || 'Bilinmiyor', // username veya name alanını kullan
      avatar: populatedAuthor.avatar || null // avatar alanını kullan, yoksa null
    };
  }, []);
  ...
  const postsForCards: BlogPostCardData[] = useMemo(() => {
    return posts.map((post) => {
        ...
        return {
            ...
            author: authorDetails.name, // Kartta gösterilecek yazar adı
            authorId: authorDetails.id, // Yazarın ID'si (yetki kontrolü için)
            authorAvatar: authorDetails.avatar, // Yazarın avatar URL'si
  ...
  const postsForSidebar: SidebarPostData[] = useMemo(() => {
     return postsForCards.slice(0, 5).map(p => ({ // Sadece gerekli alanları al
         _id: p._id,
         title: p.title,
         date: p.date,
         image: p.image, // Sidebarda görsel gösteriliyorsa
         link: p.link,
     }));
  }, [postsForCards]);
  ...
  const categoriesWithCounts = useMemo(() => {
    ...
    posts.forEach(post => { const categoryName = post.category || 'Genel'; counts[categoryName] = (counts[categoryName] || 0) + 1; });
          ...
          <div className="container px-4 sm:px-6 lg:px-8 mx-auto mt-12">
              <div className="flex flex-col lg:flex-row gap-8 lg:gap-12">
                <div className="lg:w-2/3">
                  ...
                  {userInfo?.token && ( // Sadece tokenı varsa yani giriş yapmışsa formu gösterelim
                  ...
                  <div className="space-y-8">
                    {/* BlogPostComponent yerine doğrudan kart görünümü render edildi */}
                    {postsForCards.map((cardPost) => {
                      ...
                      const isAuthor = userInfo && originalPost && originalPost.author && typeof originalPost.author === 'object' && originalPost.author._id === userInfo._id;
                      // userInfo'da role alanı varsa kullan, yoksa false say
                      ...
                      return (
                          <div key={cardPost._id} className="relative group bg-white dark:bg-gray-800 rounded-lg shadow-md hover:shadow-xl transition-shadow duration-300 border dark:border-gray-700 overflow-hidden">
                                  ...
                                  <div className="sm:w-1/3 aspect-[16/9] sm:aspect-auto overflow-hidden bg-muted flex-shrink-0">
                                      <img
                                          ...
                                          onError={(e) => {
                                            ...
                                            (e.target as HTMLImageElement).src = '/default-blog.png'; // Varsayılan resminizin yolu
                                                ...
                                                <div className="flex items-center ml-4">
                                                  ...
                                                  {cardPost.authorAvatar ? (
                                                     <Avatar className="h-6 w-6 mr-1.5">
                                                        ...
                                                        <AvatarFallback>{cardPost.author ? cardPost.author.substring(0, 2).toUpperCase() : 'YY'}</AvatarFallback>
                                                     </Avatar>
                                                  ) : (
                                                      <UserIcon className="mr-1.5 h-4 w-4" /> // Avatar yoksa User ikonu
                                                  )}
                                                  <span>{cardPost.author}</span> {/* Yazar adı */}
                                                </div>
                                              )}
                                          </div>
                              ...
                              {(isAdmin || (userInfo?.token && isAuthor)) && ( // Yetki kontrolü
