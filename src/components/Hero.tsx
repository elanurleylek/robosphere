import React, { useEffect, useRef, useState } from 'react'; // <-- useState'i ekleyin
...
const Hero: React.FC = () => {
  ...
  // 2. showMoreInfo state'ini tanımlayın
  const [showMoreInfo, setShowMoreInfo] = useState(false); // <-- Bu satırı ekleyin
  ...
  // Daha fazla bilgi metni
  const moreInfoText = "Robotikle sadece hayal etme, inşa et! Robotik Okulu, seni teorik bilgilerle donatmanın ötesine geçiyor. Arduino, Raspberry Pi gibi platformlarla gerçek robotik projeler geliştir, video derslerle adım adım öğren, ulusal yarışmalarda yeteneğini kanıtla. Robotik kariyerinde bir adım öne geçmek için ihtiyacın olan her şey burada!";
      ...
      <div className="max-w-7xl mx-auto w-full grid md:grid-cols-2 gap-12 lg:gap-20 items-center relative z-10">
        ...
        <div className="hero-content flex flex-col space-y-8 md:pr-8">
          ...
          <div className="flex flex-col sm:flex-row gap-4 animate-fade-up" style={{ animationDelay: '0.3s' }}>
            <Button
              size="lg"
              className="bg-primary text-white hover:bg-primary/90 transition-colors group"
              onClick={() => navigate('/register')}
            >
              Ücretsiz Başla
              <ArrowRight className="ml-2 h-4 w-4 group-hover:translate-x-1 transition-transform" />
            </Button>
             {/* 3. Daha Fazla Bilgi butonuna onClick ekleyin */}
            <Button
              size="lg"
              variant="outline"
              onClick={() => setShowMoreInfo(!showMoreInfo)} // <-- Bu satırı ekleyin
            >
              Daha Fazla Bilgi
            </Button>
          </div>

           {/* 4. showMoreInfo true ise yazıyı render edin */}
          {showMoreInfo && ( // <-- Bu koşullu render satırını ekleyin
            <div className="mt-8 text-foreground/90 animate-fade-in"> {/* <-- Yeni div ve stil */}
              <p className="text-base md:text-lg max-w-xl text-balance">{moreInfoText}</p> {/* <-- Yazı burada */}
            </div>
          )} {/* <-- Koşullu render sonu */}
